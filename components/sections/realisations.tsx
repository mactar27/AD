...
import { projects, projectCategories as originalCategories } from '@/lib/site-data'
...
export function Realisations() {
  ...
  const categories = t.data.projectCategories
  
  const filtered = activeIndex === 0 
    ? projects 
    : projects.filter((p) => p.category === originalCategories[activeIndex])
      ...
      <motion.div layout className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <AnimatePresence mode="popLayout">
          {filtered.map((project) => {
            const catIndex = originalCategories.indexOf(project.category)
            const translatedCat = catIndex !== -1 ? categories[catIndex] : project.category
            ...
            >
              <div className="relative aspect-[4/3] overflow-hidden">
                <Image
                  src={project.image}
                  alt={`Projet ${project.title} — ${translatedCat}`}
                  fill
              ...
              <div className="p-5">
                ...
                <p className="mt-1 text-sm text-muted-foreground">{translatedCat}</p>
              </div>
            </motion.article>
          )})}
        </AnimatePresence>
